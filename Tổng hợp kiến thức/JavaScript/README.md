...
const getStatusMessage = (statusCode) => {
  ...
  return statusMap[statusCode] ?? "Mã lỗi không xác định";
};

console.log(getStatusMessage("404")); // "Không tìm thấy"
```
## 06. Loops - Vòng lặp
...
// 1. KIỂM TRA KIỂU DỮ LIỆU CHUẨN
typeof "abc";         // "string"
Array.isArray([1,2]); // true (Đừng dùng typeof [])

// 2. CHUYỂN KIỂU NHANH
const num = +"123";     // Ép String thành Number
const bool = !! "text"; // Ép thành Boolean (true)
const str = 123 + "";   // Ép Number thành String

// 3. XỬ LÝ MẢNG SIÊU TỐC
const uniqueArr = [...new Set([1, 2, 2, 3])]; // Xóa phần tử trùng lặp
const lastItem = arr.at(-1);                  // Lấy phần tử cuối cùng

// 4. FALLBACK DỮ LIỆU AN TOÀN
// Chỉ lấy "Guest" nếu userName là null hoặc undefined
const name = userName ?? "Guest"; 
// Truy cập Object không sợ crash lỗi "Cannot read properties"
const city = user?.address?.city; 

// 5. FETCH API CƠ BẢN VỚI ASYNC/AWAIT
